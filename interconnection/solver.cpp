...
bool solver::setup(void)
{
	try {
		...
		gl_debug("creating vector x for %d lines", n_lines);
		x = vec(n_lines,fill::zeros);
		gl_debug("creating vector b for %d nodes", n_nodes);
		b = vec(n_nodes,fill::zeros);
...
bool solver::test_unconstrained(size_t N)
{
	try {
		...
		// net power matrix
		size_t M = N*(N-1)/2;
		gl_debug("creating vector b for %d nodes",N);
		b = vec(N,fill::randn);

		// connectivity vector
		gl_debug("creating vector C for %d lines",M);
		vec C(M,fill::randu);

		// graph matrix
		gl_debug("creating matrix A for %d lines and %d nodes",M,N);
